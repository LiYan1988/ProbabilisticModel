...
save('data/multiHop2_input.mat')

%% calculate noise based on bandwidths
demandsBandwidthsAll = zeros(Ndemands, Nsimu);
demandsNoiseAll = zeros(Ndemands, Nsimu);
demandsSEAll = zeros(Ndemands, Nsimu);

nIter = 1;
tic;
for k=1:Nsimu
    [demandsBandwidthsAll(:, k), demandsNoiseAll(:, k), ...
        demandsSEAll(:, k), ~] = solveBWNetwork(TopologyStruct, ...
        DemandStruct(k), systemParameters, nIter);
    fprintf('Simulation %d is done.\n', k);
end
runtime = toc;

save('data/multiHop2_result.mat')