...
dataRates1 = normrnd(100, 30, [Nuser, Nsimu]);
dataRates2 = normrnd(350, 30, [Nuser, Nsimu]);
selector = binornd(1, 0.5, [Nuser, Nsimu]);
dataRates = max(30*ones(Nuser, Nsimu), selector.*dataRates1+(1-selector).*dataRates2);
histogram(dataRates(:))
tic;
...
ylabel('ESE (bit/Hz)')
savefig('figures/singleLink10User1e3_SweepLength_MultiModuleDistribution.fig')
save('data/singleLink10User1e3_SweepLength_MultiModuleDistribution.mat')