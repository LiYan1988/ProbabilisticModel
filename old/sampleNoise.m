...
for i=1:Ndemands
    ...
    for j=1:NLinks
        ...
        Btotal(:, j) = sampleSumDistribution(distributionName, p1, p2, ...
            NumberOfDemandsOnLink(j)-1, Nsamples);
        % sample XCI 
        tmpXCI(:, j) = 2*mu*tmpLength*psd^3*log((Btotal(:, j)+Bi+2*gb)./....
            (Bi+2*gb));
        % sample ASE
...
demandALL = squeeze(sum(sampleNoiseALLPerLink, 2)).';
demandSCI = squeeze(sum(sampleNoiseSCIPerLink, 2)).';
demandNLI = squeeze(sum(sampleNoiseNLIPerLink, 2)).';
demandXCI = squeeze(sum(sampleNoiseXCIPerLink, 2)).';

SampleNoise.demandALL = demandALL;
SampleNoise.demandSCI = demandSCI;
SampleNoise.demandNLI = demandNLI;
SampleNoise.demandXCI = demandXCI;

SampleNoise.linkALL = squeeze(sum(sampleNoiseALLPerLink, 3)).';
SampleNoise.linkSCI = squeeze(sum(sampleNoiseSCIPerLink, 3)).';
SampleNoise.linkXCI = squeeze(sum(sampleNoiseXCIPerLink, 3)).';
SampleNoise.linkNLI = squeeze(sum(sampleNoiseNLIPerLink, 3)).';
