...
%% Import the data
[~, ~, raw] = xlsread('CORONET_CONUS_Topology.xls','Nodes');
raw = raw(6:end,:);
...
%% Import the data
[~, ~, raw] = xlsread('CORONET_CONUS_Topology.xls','Links');
raw = raw(6:end,:);
stringVectors = string(raw(:,[1,2]));
stringVectors(ismissing(stringVectors)) = '';
raw = raw(:,3);
...
csvwrite('CoronetCostMatrix.csv', networkCostMatrix)