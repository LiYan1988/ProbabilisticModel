...
% Test BP with the same #RSs between the RS and RC methods and Predo's
% routing only algorithm, Tab. 4 in the ECOC paper. 
% Simulate as many traffic matrices (at least 20) as possible, given #RSs
% (#RSs = 10, 15, 20, 25, 30)
clc;
clear;
close all;
...
nsec = 0;
partition = 'economy';
account = 'maite_group'; % optinetly6j or maite_group
...
nArray = 2; % how many array jobs/traffic matrices per alg per #RSs
NMonteCarlo = 2; % #shuffles per traffic matrices
% alg = {'proposed_RS_MD', 'proposed_RC_MD', 'benchmark_Bathula_MD', ...
%     'benchmark_Predo_routingReach', 'benchmark_Predo_routingOnly'};
alg = {'proposed_RS_MD', 'proposed_RC_MD', ...
    'benchmark_Predo_routingReach', 'benchmark_Predo_routingOnly'};
startNum = 5; % 10, 15, 20, 25, 30
nRSVector = startNum:5:startNum; % Note: nRSVector is actually a number
folderName = sprintf('PredoRoutingOnly_nRS_%d_1', startNum);
