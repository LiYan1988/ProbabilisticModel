...
alg = 'benchmark2';
nRS = 20;
simulationName = sprintf('mc2b1_bm2_%d', nRS);
NMonteCarlo = 2; % number of simulations in one Monte Carlo simulation
...
for arrayId = 1:nArray
    ...
    fid = fopen('template_file_block_probability2.m','r');
    i = 1;
    tline = fgetl(fid);
    A{i} = tline;
    while ischar(tline)
